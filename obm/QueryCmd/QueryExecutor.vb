...
Namespace Query
    Public MustInherit Class QueryExecutor
        ...
        Public Class cls
            ...
            Private _pkFields As IEnumerable(Of String)
            ...
            Private moreThanOneType As Boolean
            Public Property Cancel() As Boolean
                Get
                    Return moreThanOneType
                End Get
                Set(ByVal value As Boolean)
                    moreThanOneType = value
                End Set
            End Property
...
l1:
                            If sender._sl IsNot Nothing Then

                                'For Each se As SelectExpression In sender._sl
                                '    If (se.Attributes And Field2DbRelations.PK) = Field2DbRelations.PK Then
                                '        _pk = se.GetIntoPropertyAlias
                                '    End If
                                'Next
                                _oschema = New SimpleObjectSchema(SelectExpression.GetMapping(sender._sl, _mgr.MappingEngine, sender))
                                Dim pk = _oschema.GetPK
                                If pk IsNot Nothing Then
                                    _pkFields = pk.SourceFields.Select(Function(it) it.SourceFieldExpression)
                                End If
                            End If
                            ...
                            For Each su As SelectUnion In Expressions2.Helper.GetSelectedEntities(se)
                                ...
                                If d IsNot Nothing Then
                                    ...
                                    If firstType Is Nothing Then
                                        firstType = rt
                                    ElseIf Not moreThanOneType Then
                                        moreThanOneType = firstType IsNot rt
                                    End If
                                End If
                            Next
                        Next
l2:
                        args.Cancel = moreThanOneType
                        'If Not _cancel Then
        ...
        Protected Delegate Function GetCachedItemDelegate(
        ...
        Protected Delegate Function GetListFromCachedItemDelegate(Of ReturnType)(
            ByVal mgr As OrmManager, ByVal query As QueryCmd, ByVal cacheItemProvider As OrmManager.ICacheItemProvoderBase,
        ...
        Protected MustOverride Function GetProvider(ByVal mgr As OrmManager, ByVal query As QueryCmd,
            ByVal d As InitTypesDelegate) As CacheItemBaseProvider

        Protected MustOverride Function _Exec(Of ReturnType)(ByVal mgr As OrmManager,
        ...
        Public Function Exec(ByVal mgr As OrmManager, ByVal query As QueryCmd) As ReadonlyMatrix Implements IExecutor.Exec
            ...
            Dim res As ReadonlyMatrix = _Exec(Of ReadonlyMatrix)(mgr, query,
                Function() GetCacheItemProvider(mgr, query),
                Function(m As OrmManager, q As QueryCmd, dic As IDictionary, id As String, sync As String, cacheItemProvider As OrmManager.ICacheItemProvoderBase) _
                    m.GetFromCacheBase(dic, sync, id, Nothing, cacheItemProvider, Nothing),
        ...
        Public Function Exec(Of ReturnType As ICachedEntity)(ByVal mgr As OrmManager, ByVal query As QueryCmd) As ReadOnlyEntityList(Of ReturnType) Implements IExecutor.Exec
            ...
            Try
                ...
                Dim cis As New CustomInfoStore
                Dim res As ReadOnlyEntityList(Of ReturnType) = _Exec(Of ReadOnlyEntityList(Of ReturnType))(mgr, query,
                   Function() GetCacheItemProvoder(Of ReturnType)(mgr, query),
                   Function(m As OrmManager, q As QueryCmd, dic As IDictionary, id As String, sync As String, cacheItemProvider As OrmManager.ICacheItemProvoderBase) _
                       m.GetFromCache(Of ReturnType)(dic, sync, id, q.propWithLoad, cacheItemProvider),
        ...
        Public Function Exec(Of CreateType As {New, ICachedEntity}, ReturnType As ICachedEntity)(ByVal mgr As OrmManager, ByVal query As QueryCmd) As ReadOnlyEntityList(Of ReturnType) Implements IExecutor.Exec
            ...
            Try
                ...
                Dim cis As New CustomInfoStore
                Dim res As ReadOnlyEntityList(Of ReturnType) = _Exec(Of ReadOnlyEntityList(Of ReturnType))(mgr, query,
                   Function() GetCacheItemProvoderT(Of CreateType, ReturnType)(mgr, query),
                   Function(m As OrmManager, q As QueryCmd, dic As IDictionary, id As String, sync As String, cacheItemProvider As OrmManager.ICacheItemProvoderBase) _
                       m.GetFromCache(Of ReturnType)(dic, sync, id, q.propWithLoad, cacheItemProvider),
                   AddressOf cis.GetEntityList(Of ReturnType))
        ...
        Private Function _ExecEntity(Of ReturnType As {Entities._IEntity})(ByVal mgr As OrmManager, ByVal query As QueryCmd,
            ...
            Try
                AddHandler query.QueryPrepared, AddressOf c.QueryPreparedHandler
                Dim cis As New CustomInfoStore
                Dim res As ReadOnlyObjectList(Of ReturnType) = _Exec(Of ReadOnlyObjectList(Of ReturnType))(mgr, query, d,
                    Function(m As OrmManager, q As QueryCmd, dic As IDictionary, id As String, sync As String, cacheItemProvider As OrmManager.ICacheItemProvoderBase) _
                        m.GetFromCache2(dic, sync, id, True, cacheItemProvider),
        ...
        Public Function ExecEntity(Of CreateType As {New, Entities._IEntity}, ReturnType As Entities._IEntity)(
            ...
            If GetType(AnonymousCachedEntity).IsAssignableFrom(GetType(CreateType)) Then
                ...
                Try
                    AddHandler query.QueryPrepared, AddressOf c.QueryPreparedHandler
                    Return _ExecEntity(Of ReturnType)(mgr, query,
        ...
        Public Function ExecSimple(Of ReturnType)(ByVal mgr As OrmManager, ByVal query As QueryCmd) As IList(Of ReturnType) Implements IExecutor.ExecSimple
            ...
            Try
                Dim cis As New CustomInfoStore
                Dim res As IList(Of ReturnType) = _Exec(mgr, query, Function() GetCacheItemProvoderS(Of ReturnType)(mgr, query),
                    Function(m As OrmManager, q As QueryCmd, dic As IDictionary, id As String, sync As String, p2 As OrmManager.ICacheItemProvoderBase) _
                        m.GetFromCacheBase(dic, sync, id, Nothing, p2, Nothing),
