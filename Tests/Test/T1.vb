...
Namespace test
	...
	<Worm.Orm.EntityAttribute(GetType(test.T1.T1SchemaDef), "1", EntityName:="T1")>  _
	Public Class T1
		Inherits Worm.Orm.OrmBase
		
		#Region "Private Fields"
		Private _i As System.Nullable(Of Integer)
		...
		<Worm.Orm.ColumnAttribute("I", Worm.Orm.Field2DbRelations.None)>  _
		Public Overridable Property I() As System.Nullable(Of Integer)
			Get
				Dim syncHelper As System.IDisposable
				syncHelper = Nothing
				Try 
					syncHelper = Me.SyncHelper(true, "I")
					Return Me._i
				Finally
			...
			Set
				...
				syncHelper = Nothing
				Try 
					syncHelper = Me.SyncHelper(false, "I")
					Me._i = value
				Finally
					If (Not (syncHelper) Is Nothing) Then
						CType(syncHelper,System.IDisposable).Dispose
		...
		<Worm.Orm.ColumnAttribute("S", Worm.Orm.Field2DbRelations.None)>  _
		Public Overridable Property S() As String
			Get
				Dim syncHelper As System.IDisposable
				syncHelper = Nothing
				Try 
					syncHelper = Me.SyncHelper(true, "S")
					Return Me._s
				Finally
					If (Not (syncHelper) Is Nothing) Then
						CType(syncHelper,System.IDisposable).Dispose
			...
			Set
				...
				syncHelper = Nothing
				Try 
					syncHelper = Me.SyncHelper(false, "S")
					Me._s = value
				Finally
					If (Not (syncHelper) Is Nothing) Then
						CType(syncHelper,System.IDisposable).Dispose
		...
		'''</summary>
		Protected Shared Sub CopyT1(ByVal from As test.T1, ByVal [to] As test.T1)
			[to]._i = from._i
			[to]._s = from._s
		End Sub
		#End Region
		...
		Protected Overrides Sub CopyBody(ByVal from As Worm.Orm.OrmBase, ByVal [to] As Worm.Orm.OrmBase)
			test.T1.CopyT1(CType(from,test.T1), CType([to],test.T1))
		End Sub
		...
		Public Overrides Sub SetValue(ByVal pi As System.Reflection.PropertyInfo, ByVal c As Worm.Orm.ColumnAttribute, ByVal value As Object)
			If (c.FieldName = "I") Then
				Me._i = CType(value,System.Nullable(Of Integer))
				Return
			End If
			If (c.FieldName = "S") Then
				Me._s = CType(value,String)
				Return
			End If
		...
		Protected Overrides Function GetNew() As Worm.Orm.OrmBase
			Return New test.T1(Me.Identifier, Me.OrmCache, Me.OrmSchema)
		End Function
		
		#Region "Nested Types"
		Public Class T1SchemaDef
			...
			Public Overridable Function GetTables() As Worm.Orm.OrmTable() Implements Worm.Orm.IOrmObjectSchema.GetTables
				If (Me._tables Is Nothing) Then
					Dim lockCachedExpression_0836b2c1d9664f729f6a83d46fc6c862 As Object = Me._forTablesLock
					System.Threading.Monitor.Enter(lockCachedExpression_0836b2c1d9664f729f6a83d46fc6c862)
					Try 
						If (Me._tables Is Nothing) Then
							Me._tables = New Worm.Orm.OrmTable() {New Worm.Orm.OrmTable("[dbo].[t1]")}
						End If
					Finally
						System.Threading.Monitor.Exit(lockCachedExpression_0836b2c1d9664f729f6a83d46fc6c862)
					End Try
				End If
				Return Me._tables
			End Function
			
			Protected Overridable Function GetTable(ByVal tbl As test.T1.T1SchemaDef.TablesLink) As Worm.Orm.OrmTable
			...
			Public Overridable Function GetM2MRelations() As Worm.Orm.M2MRelation() Implements Worm.Orm.IOrmObjectSchema.GetM2MRelations
				If (Me._m2mRelations Is Nothing) Then
					Dim lockCachedExpression_1c16c062763045ab936072beb2285c7e As Object = Me._forM2MRelationsLock
					System.Threading.Monitor.Enter(lockCachedExpression_1c16c062763045ab936072beb2285c7e)
					Try 
						If (Me._m2mRelations Is Nothing) Then
							Me._m2mRelations = New Worm.Orm.M2MRelation(-1) {}
						End If
					Finally
						System.Threading.Monitor.Exit(lockCachedExpression_1c16c062763045ab936072beb2285c7e)
					End Try
				End If
			...
			Public Overridable Function GetFieldColumnMap() As Worm.Orm.Collections.IndexedCollection(Of String, Worm.Orm.MapField2Column) Implements Worm.Orm.IOrmObjectSchema.GetFieldColumnMap
				If (Me._idx Is Nothing) Then
					Dim lockCachedExpression_f5f16dda4bd441348aaa464777cab65d As Object = Me._forIdxLock
					System.Threading.Monitor.Enter(lockCachedExpression_f5f16dda4bd441348aaa464777cab65d)
					Try 
						If (Me._idx Is Nothing) Then
							Dim idx As Worm.Orm.OrmObjectIndex = New Worm.Orm.OrmObjectIndex
							idx.Add(New Worm.Orm.MapField2Column("I", "i", Me.GetTable(test.T1.T1SchemaDef.TablesLink.tbldbot1)))
							idx.Add(New Worm.Orm.MapField2Column("S", "s", Me.GetTable(test.T1.T1SchemaDef.TablesLink.tbldbot1)))
							Me._idx = idx
						End If
					Finally
						System.Threading.Monitor.Exit(lockCachedExpression_f5f16dda4bd441348aaa464777cab65d)
					End Try
				End If
				Return Me._idx
			End Function
